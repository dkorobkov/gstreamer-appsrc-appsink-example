...
{
	GMainLoop *loop;
	GstElement *pipeline, *nvjpegenc;
	GstElement *queue1, *nvvidconv, *queue2;
	GstBus *bus;
	guint bus_watch_id;
	...
	gst-launch-1.0 -v filesrc location=g8.raw blocksize=2073600 ! 'video/x-raw,width=1920,height=1080,format=GRAY8,framerate=0/1' !
	 	 	 nvjpegenc quality=90 ! filesink location=gstest.jpg

    To use TEGRA X1 hardware JPEG encoding, nvvidconv must be added:

    gst-launch-1.0 -v filesrc location=g8.raw blocksize=2073600 ! 'video/x-raw,width=1920,height=1080,framerate=1/1,format=GRAY8' ! queue !
			nvvidconv ! 'video/x-raw(memory:NVMM)' ! queue ! nvjpegenc quality=90 ! filesink location=gstest.jpg

	blocksize must be exactly one raw frame, this will allow not using videoparse and queue in pipeline.
	It means full buffers are fed to nvjpegenc at once. Pipeline was tested with filesrc and filesink first.
	*/

	pipeline = gst_pipeline_new("mypipeline");
	appsrc = gst_element_factory_make("appsrc", "mysource");
	queue1 = gst_element_factory_make("queue", "myqueue1");
	nvvidconv = gst_element_factory_make("nvvidconv", "mynvvidconv");
	queue2 = gst_element_factory_make("queue", "myqueue2");
	nvjpegenc = gst_element_factory_make("nvjpegenc", "myenc");
	appsink = gst_element_factory_make("appsink", "mysink");

	// Check if all elements were created
	if (!pipeline || !appsrc || !queue1 || !queue2 !! !nvvidconv !! !nvjpegenc || !appsink)
	{
	...
	// appsrc should be linked to nvjpegenc with these caps otherwise nvjpegenc does not know size of incoming buffer
	GstCaps *capsappsrc2queue1; // between appsrc and nvjpegenc
	capsappsrc2queue1 = gst_caps_new_simple("video/x-raw", "format",
			G_TYPE_STRING, "GRAY8", "width", G_TYPE_INT, width, "height",
			G_TYPE_INT, height, "framerate", GST_TYPE_FRACTION, 0, 1, NULL);

	// nvvidconv should be linked to queue2 with these caps
	GstCaps *capsnvvidconv2queue = NULL;
	capsnvvidconv2queue = gst_caps_new_simple("video/x-raw(memory:NVMM)", NULL);

	// blocksize is important for nvjpegenc to know how many data to expect from appsrc in a single frame, too
	char szTemp[64];
	...
	g_object_set(G_OBJECT (nvjpegenc), "quality", quality, NULL);
	...
	gst_bin_add_many(GST_BIN(pipeline), appsrc, 
			queue1, nvvidconv, queue2,
			nvjpegenc, appsink, NULL);

	// link elements into pipe: appsrc -> queue1 -> nvvidconv -> queue2 -> nvjpegenc -> appsink
	gst_element_link_filtered(appsrc, queue1, capsappsrc2queue1);
	gst_element_link(queue1, nvvidconv);
	gst_element_link_filtered(nvvidconv, queue2, capsnvvidconv2queue);
	gst_element_link(queue2, nvjpegenc);
	gst_element_link(nvjpegenc, appsink);
